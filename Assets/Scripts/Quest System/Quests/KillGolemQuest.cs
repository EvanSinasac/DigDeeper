using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class KillGolemQuest : Quest
{
    // Start is called before the first frame update
    void Start()
    {
        QuestName = "Golems to Crush";
        QuestType = "KillGolemQuest";
        Description = "Kill 3 Golems";
        Reward = GameManager.instance.itemDatabase.GetItemReferenceName("Coffee");
        //Debug.Log(Reward.name);
        RewardAmount = 5; //5 coffee

        Goals.Add(new KillGoal(this, 4, "Kill 3 Golems", false, 0, 3));
        //Goals.Add(new CollectionGoal(this, "Slime", "Collect 20 slime", false, 0, 20));

        Goals.ForEach(g => g.Init());
    }

}
